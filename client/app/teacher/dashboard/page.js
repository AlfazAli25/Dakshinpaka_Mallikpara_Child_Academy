...
export default function TeacherDashboardPage() {
  ...
  const downloadReceiptPdf = async (paymentId, receiptNumber) => {
    ...
    try {
      const blob = await getBlob(`/receipts/teacher/${normalizedPaymentId}`, token, { timeoutMs: 120000 });
      const safeReceiptToken = String(receiptNumber || normalizedPaymentId).replace(/[^A-Za-z0-9_-]/g, '_');
      downloadBlob(blob, `Salary_Receipt_${safeReceiptToken}.pdf`);
    } catch (_error) {
      setReceiptDownloadError('Failed to download receipt');
    } finally {
        ...
        {loading ? (
          ...
          <div className="mt-3 max-h-[260px] space-y-2 overflow-y-auto pr-1">
            {receipts.map((receipt) => {
              const paymentId = String(receipt?.payrollId || receipt?._id || '').trim();
